...
宏是全局预处理符号，不受 namespace 保护。ZeroStyle 默认只暴露 `ZERO_` 前缀宏，例如 `ZERO_NODISCARD`、`ZERO_REQUIRES`。

短名字宏如 `NODISCARD`、`REQUIRES` 只在显式定义 `ZERO_ENABLE_SHORT_MACROS` 后启用，适合个人项目或受控项目边界，不作为默认低侵入接口。
